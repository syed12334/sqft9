 ...
 <div class="container-fluid my-3">
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <div class="card-title">Property Amenity</div>
                        <?php 
                            if($this->session->flashdata('message')) {
                                echo $this->session->flashdata('message');
                            }
                        ?>
                        <a class="btn btn-success" href="<?= base_url().'addamenity';?>" style="float: right;margin-bottom: 20px;margin-right: 20px"><i class="fa fa-plus"></i> Add</a>
                        <br /><br /><br />
    ...
    var dataTable, edit_data;
            function initialiseData(){
                dataTable = $('#category_table').DataTable({  
                    ...
                    "order":[],  
                    "ajax":{  
                        url:"<?=base_url().'master/getPropertyamenity'?>",  
                        type:"POST",
                        data: function(d){
            ...
            initialiseData();
                function updateStatus(id,status){
                       var postdata = { id : id,status : status,"<?= $this->security->get_csrf_token_name();?>":$(".csrf_token").val() } ;
                        ...
                        $.ajax({                        
                            url: "<?=base_url().'master/setpropertyamenityStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
